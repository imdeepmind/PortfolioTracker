...
export default function TransactionsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await fetch(`/api/holdings/${holdingId}/transactions/${deleteTarget}`, {
        method: 'DELETE',
      });
              ...
              <tbody>
                {transactions.map((tx) => {
                  ...
                  const gainLossPercent =
                    tx.totalAmountInvested !== 0 ? (gainLoss / tx.totalAmountInvested) * 100 : 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-sm text-white text-right font-medium whitespace-nowrap">
                        {formatCurrency(tx.amount)}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-300 text-right whitespace-nowrap">
                        {formatCurrency(tx.totalAmountInvested)}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-300 text-right whitespace-nowrap">
                        {formatCurrency(tx.totalPortfolioSize)}
                      </td>
